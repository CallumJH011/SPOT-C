...
def format_time_full(ms):
    ...
    if ms is None: return "00:00.000"
    s, ms = divmod(int(ms), 1000)
    m, s = divmod(s, 60)
    return f"{m:02}:{s:02}.{ms:03}"

def parse_time_to_ms(time_str):
    """Converts MM:SS.mmm string back to integer milliseconds"""
    try:
...
QPushButton#PrimaryBtn {{ 
    background-color: #009DDC; color: white; border-radius: 6px; 
    padding: 10px; font-weight: bold; border: none; 
}}
QPushButton#PrimaryBtn:hover {{ background-color: #008AC2; }}
QPushButton#PrimaryBtn:disabled {{ background-color: #BDC3C7; color: #ECF0F1; }}
...
class InfoDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("About SPOT-C")
        self.setFixedSize(450, 320)
        layout = QVBoxLayout(self)
        
        title = QLabel("<h2>SPOT-C</h2>")
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)

        desc = QLabel(
            "<b>Description:</b><br>"
            "SPOT-C is a clinical annotation tool designed to help Speech Pathology "
            "practitioners analyse video sessions with structured qualitative data. "
            "Tag behaviors in real-time and export session logs for clinical review."
        )
        ...
        authors = QLabel(
            "<b>Authors:</b><br>"
            "Developed by Juliette Dwyer & Callum Harsas<br>"
            "Contact: callum.harsas@sydney.edu.au<br>"
...
class EditTagDialog(QDialog):
    def __init__(self, tag_data, player_ref, parent=None):
        ...
        self.start_in = QLineEdit(format_time_full(tag_data['Start']))
        self.end_in = QLineEdit(format_time_full(tag_data['End']))
        sk = QPushButton("Seek"); sk.setObjectName("SecondaryBtn")
        sk.clicked.connect(lambda: self.player.set_time(parse_time_to_ms(self.start_in.text())))
        t_layout.addWidget(QLabel("Start:")); t_layout.addWidget(self.start_in)
        t_layout.addWidget(QLabel("End:")); t_layout.addWidget(self.end_in)
    ...
    def get_data(self):
        ...
        return parse_time_to_ms(self.start_in.text()), parse_time_to_ms(self.end_in.text()), cats, p
...
class VideoTagger(QMainWindow):
    ...
    def init_ui(self):
        ...
        l_row = QHBoxLayout()
        l_btn = QPushButton("Load Recording"); l_btn.setObjectName("PrimaryBtn"); l_btn.clicked.connect(self.load_video)
        self.ls_btn = QPushButton("Load from Save"); self.ls_btn.setObjectName("PrimaryBtn")
        self.ls_btn.setEnabled(False); self.ls_btn.clicked.connect(self.load_from_csv)
        l_row.addWidget(l_btn); l_row.addWidget(self.ls_btn); side_layout.addLayout(l_row)
        ...
        self.mark_btn = QPushButton("Start Tagging"); self.mark_btn.setObjectName("SecondaryBtn")
        ...
        # Info Button
        info_btn = QPushButton("Info"); info_btn.setObjectName("SecondaryBtn")
        info_btn.clicked.connect(lambda: InfoDialog(self).exec())
    ...
    def load_video(self):
        ...
        if path:
            ...
            self.mediaplayer.play(); QTimer.singleShot(200, self.mediaplayer.pause); self.ls_btn.setEnabled(True)
    ...
    def mark_start(self):
        ...
        self.mark_btn.setText(f"Tagging... ({format_time_full(self.start_time)})")
        self.mark_btn.setStyleSheet("background-color: #E74C3C; color: white; font-weight: bold; border-radius: 6px;")
    ...
    def save_tag(self):
        ...
        self.update_stats(); self.start_time = None
        # Reset button
        self.mark_btn.setText("Start Tagging")
